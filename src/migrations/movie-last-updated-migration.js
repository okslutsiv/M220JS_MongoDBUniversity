...
;(async () => {
  try {
    ...
    const host =
      "mongodb+srv://m220student:m220password@mflix-v23ei.mongodb.net/test?retryWrites=true&w=majority"

    const client = await MongoClient.connect(host, {
      useNewUrlParser: true,
      useUnifiedTopology: true,
    })
    //const mflix = client.db(process.env.MFLIX_NS) - no access to process.env from here, needed database name
    console.log(process.env.MFLIX_NS) // undefined
    ...
    // over the wire!
    const predicate = {
      lastupdated: { $exists: true, $type: "date" },
    }
